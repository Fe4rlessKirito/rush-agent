...
# ---- Image Analysis ---------------------------------------------------------
# Enable image upload/analysis
IMAGE_ANALYSIS_ENABLED = True

# Max image size for uploads (in bytes)
MAX_IMAGE_SIZE = 10 * 1024 * 1024  # 10MB

# Supported image formats
SUPPORTED_IMAGE_FORMATS = ["image/png", "image/jpeg", "image/gif", "image/webp", "image/avif"]
...
ACCOUNT_POOL_SIZE = 30                                   # ready accounts kept warm
ACCOUNT_POOL_REFILL_SEC = 3                             # how often to top the pool up
ACCOUNT_TTL_SEC = 1800                                   # drop pooled accounts older than this
...
# ---- Runtime config (editable live via GET /config + POST /config) ----------
# These values can be changed without restarting the server. The /config
# endpoints read and write this block directly to config.py on disk.
RUNTIME_POOL_SIZE       = 30      # accounts kept warm in the pool (mirrors ACCOUNT_POOL_SIZE)
RUNTIME_SIGNUP_DELAY_MS = 0       # ms to wait between account signups (0 = as fast as possible)
RUNTIME_ACCOUNT_TTL_SEC = 1800    # seconds before a pooled account is rotated out
...
TOR_BROWSER_DIR = r"C:\Users\marko\OneDrive\Desktop\Tor Browser\Browser\firefox.exe"   # your Tor Browser folder
