...
async def stream_messages(model: str, messages: list, acct: dict | None = None):
    ...
    if direct.enabled():
        ...
        try:
            if acct is None:
                acct = await account_pool.POOL.acquire()
            log.info(f"Starting direct.stream for model={model}")
            async for delta in direct.stream(model, messages=messages, acct=acct):
                produced = True
                log.info(f"direct.stream yielded: {delta[:40]!r}")
                yield delta
            health.H.send(True, "direct")
            log.info(f"direct.stream completed, produced={produced}")
            return
        except Exception as e:
            health.H.send(False, "direct", e)
            if produced:
                log.info(f"direct.stream had partial output, returning")
                return
            if not getattr(config, "BROWSER_FALLBACK_ENABLED", False):
    ...
    log.info("Falling back to cold run (non-streaming)")
    reply = await _cold_run(model, _flatten_messages(messages))
    health.H.send(True, "cold")
    log.info(f"Cold run complete, reply length: {len(reply)}")
    yield reply